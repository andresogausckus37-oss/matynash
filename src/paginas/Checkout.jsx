...
export default function Checkout() {
  ...
  // TOTALS
  const totalOriginalARS = carrito.reduce((sum, item) => sum + item.precioARS, 0);
  const totalFinalARS = carrito.reduce((sum, item) => sum + calcularPrecioConDescuento(item), 0);
  const totalUSD = carrito.reduce((sum, item) => sum + (Number(item.precioUSD) || 0), 0);
  const cuotas = carrito[0]?.cuotasSinInteres || 3;
  const valorCuota = Math.round(totalFinalARS / cuotas);
  const descuentoGlobal = carrito.some(item => item.descuentoPorcentaje);
  const primerDescuento = carrito.find(item => item.descuentoPorcentaje)?.descuentoPorcentaje;
    ...
    return (
      <main className="max-w-2xl mx-auto px-4 py-16 text-center">
        <div className="bg-white border border-borde rounded-xl p-8 shadow-suave">
          <h2 className="text-2xl font-semibold text-texto mb-3">Tu carrito está vacío</h2>
          <p className="text-texto-suave mb-8">Selecciona un servicio para continuar.</p>
          <Link
            to="/servicios"
            className="inline-flex items-center justify-center bg-primario hover:bg-primario-oscuro text-white px-6 py-3 rounded-lg font-medium transition-colors"
          >
            Ver servicios
          </Link>
        </div>
  ...
  // MENSAJE WHATSAPP — VERSIÓN CÁLIDA Y NATURAL
  const generarMensajeWhatsApp = (paypalPagado = false) => {
    const servicios = carrito.map((item) => {
      ...
      if (metodoPago === "paypal") {
        return `Soy ${nombre.trim()} y quiero reservar la sesión de ${item.titulo} para el día ${fecha} a las ${hora} con un valor de $${item.precioUSD} USD\n\nAdjunto comprobante de pago y espero la confirmación. Gracias 🙏🌿`;
      }
      return `Soy ${nombre.trim()} y quiero reservar la sesión de ${item.titulo} para el día ${fecha} a las ${hora} con un valor de $${formatearARS(precioFinal)}\n\nEspero el enlace de pago para confirmar mi lugar. Gracias 🙏🌿`;
    }).join("\n\n");
  ...
  // OPEN WHATSAPP
  const abrirWhatsApp = (paypalPagado = false) => {
    const mensaje = generarMensajeWhatsApp(paypalPagado);
  ...
  const enviarPedido = (e) => {
    ...
    if (metodoPago === "mercado-pago") return abrirWhatsApp(false);
    if (metodoPago === "paypal") setMostrarModalPayPal(true);
  };
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      ...
      >
        {/* LEFT COLUMN — RESUMEN DEL SERVICIO */}
        ...
        {/* RIGHT COLUMN — DATOS + PAGO (CON PRECIOS DENTRO) */}
        <div className="bg-white rounded-xl shadow-suave p-5 sm:p-6 lg:sticky lg:top-24 border border-borde">
          ...
          <div className="space-y-4">
            ...
            {/* MÉTODOS DE PAGO — CON PRECIOS DENTRO ✨ */}
            <div className="pt-2">
              <p className="block text-sm font-medium text-texto mb-3">Método de pago</p>
              <div className="space-y-4">
                
                {/* MERCADO PAGO — CON PRECIOS Y DESCUENTO DENTRO */}
                <label
                  className={`block p-4 rounded-lg border-2 cursor-pointer transition-all overflow-hidden focus:outline-none focus:ring-0 ${
                ...
                >
                  ...
                  <div className="flex items-start gap-3">
                    <div className="mt-1 w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0"
                      style={{ borderColor: metodoPago === "mercado-pago" ? "#009EE3" : "#ccc" }}
                    >
                      {metodoPago === "mercado-pago" && (
                    ...
                    <div className="flex-1">
                      <img
                        src="https://i.postimg.cc/HnvvbP1F/Mercado-Pago-svg.png"
                        alt="Mercado Pago"
                      ...
                      <div className="flex items-baseline gap-3 mb-2">
                        ...
                        {descuentoGlobal && primerDescuento && (
                          <span className="bg-red-100 text-red-700 text-xs font-medium px-2 py-0.5 rounded-sm">
                            {primerDescuento}% OFF
                          </span>
                        )}
                      </div>
                      <p className="text-sm text-[#009EE3] font-medium">
                        Hasta {cuotas} cuotas sin interés de ${formatearARS(valorCuota)}
                      </p>
                    </div>
                  </div>
                </label>

                {/* PAYPAL — CON PRECIO DENTRO */}
                <label
                  className={`block p-4 rounded-lg border-2 cursor-pointer transition-all overflow-hidden focus:outline-none focus:ring-0 ${
                    metodoPago === "paypal"
                ...
                >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <img
                        src="https://i.postimg.cc/T3NcSZ08/9309.png"
                        alt="PayPal"
                        className="h-12 w-auto object-contain mb-3"
                      />
                      {/* PRECIO DENTRO ✅ */}
                      <p className="text-2xl font-semibold text-texto mb-1">
                        ${totalUSD} USD
                      </p>
...
>
  {metodoPago === "paypal" ? (
    <>
      <img 
        src="https://i.postimg.cc/fbVTjgBw/9343.png" 
        alt="PayPal" 
        ...
        <div className="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-4">
          <div className="relative bg-white w-full max-w-md max-h-[90vh] overflow-y-auto rounded-xl shadow-xl p-6">
            ...
            <div className="bg-fondo rounded-lg p-4 mb-4">
              ...
              <p className="text-2xl font-medium text-texto">${totalUSD} USD</p>
            </div>
